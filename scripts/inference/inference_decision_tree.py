...
dt_estimator = joblib.load(root_folder + '/trained_models/dt_receipt_match_model.pkl')

def match_batch_receipt_txn(data):
    try:
        input_data = data.copy()
        X = input_data[['DateMappingMatch', 'TimeMappingMatch', 'PredictedTimeCloseMatch',
            'DescriptionMatch', 'ShortNameMatch', 'PredictedNameMatch',
            'AmountMappingMatch', 'PredictedAmountMatch']].values
        input_data['match_score'] = dt_estimator.predict_proba(X)[:, 1]
...
def match_new_receipt(uploaded_receipt):
    try:
        ...
        X = tdf[['DateMappingMatch', 'TimeMappingMatch', 'PredictedTimeCloseMatch',
                'DescriptionMatch', 'ShortNameMatch', 'PredictedNameMatch',
                'AmountMappingMatch', 'PredictedAmountMatch']].values
        tdf['match_score'] = dt_estimator.predict_proba(X)[:, 1]
